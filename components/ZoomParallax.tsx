import React, { useRef } from 'react';
import { useScroll, useTransform, motion } from 'framer-motion';

export default function ZoomParallax() {
    const container = useRef(null);
    const { scrollYProgress } = useScroll({
        target: container,
        offset: ['start start', 'end end']
    });

    // Scale transforms for parallax effect
    const scales = [
        useTransform(scrollYProgress, [0, 1], [1, 4]),
        useTransform(scrollYProgress, [0, 1], [1, 6]),
        useTransform(scrollYProgress, [0, 1], [1, 9]),
        useTransform(scrollYProgress, [0, 1], [1, 14]),
        useTransform(scrollYProgress, [0, 1], [1, 20]),
    ];

    const textScale = useTransform(scrollYProgress, [0, 0.8], [1, 40]);
    const textOpacity = useTransform(scrollYProgress, [0, 0.6, 0.8], [1, 1, 0]);
    const opacityFade = useTransform(scrollYProgress, [0, 0.9, 1], [1, 1, 0]);

    // 12 images for a fuller parallax experience
    const pictures = [
        // Layer 1 - Background (slowest)
        {
            src: "https://picsum.photos/600/800?random=1",
            scale: scales[0],
            rotation: -5,
            className: "w-[22vw] h-[22vh] -top-[32vh] -left-[28vw]"
        },
        {
            src: "https://picsum.photos/800/600?random=2",
            scale: scales[0],
            rotation: 4,
            className: "w-[25vw] h-[20vh] -top-[28vh] left-[22vw]"
        },
        // Layer 2
        {
            src: "https://picsum.photos/500/700?random=3",
            scale: scales[1],
            rotation: -3,
            className: "w-[18vw] h-[28vh] top-[22vh] -left-[28vw]"
        },
        {
            src: "https://picsum.photos/700/500?random=4",
            scale: scales[1],
            rotation: 5,
            className: "w-[20vw] h-[20vh] top-[28vh] left-[25vw]"
        },
        // Layer 3
        {
            src: "https://picsum.photos/600/600?random=5",
            scale: scales[2],
            rotation: 2,
            className: "w-[15vw] h-[16vh] -top-[12vh] -left-[38vw]"
        },
        {
            src: "https://picsum.photos/650/450?random=6",
            scale: scales[2],
            rotation: -4,
            className: "w-[18vw] h-[15vh] top-[8vh] left-[35vw]"
        },
        // Layer 4
        {
            src: "https://picsum.photos/500/500?random=7",
            scale: scales[3],
            rotation: 6,
            className: "w-[12vw] h-[12vh] -top-[38vh] left-[8vw]"
        },
        {
            src: "https://picsum.photos/450/650?random=8",
            scale: scales[3],
            rotation: -2,
            className: "w-[14vw] h-[18vh] top-[35vh] -left-[12vw]"
        },
        // Layer 5 - Foreground (fastest)
        {
            src: "https://picsum.photos/400/400?random=9",
            scale: scales[4],
            rotation: 8,
            className: "w-[10vw] h-[10vh] top-[5vh] -left-[18vw]"
        },
        {
            src: "https://picsum.photos/400/600?random=10",
            scale: scales[4],
            rotation: -6,
            className: "w-[11vw] h-[14vh] -top-[18vh] left-[15vw]"
        },
        // Extra images for depth
        {
            src: "https://picsum.photos/550/550?random=11",
            scale: scales[2],
            rotation: -8,
            className: "w-[14vw] h-[14vh] top-[15vh] left-[42vw]"
        },
        {
            src: "https://picsum.photos/480/680?random=12",
            scale: scales[3],
            rotation: 10,
            className: "w-[13vw] h-[16vh] -top-[25vh] -left-[42vw]"
        },
    ];

    return (
        <div ref={container} className="h-[280vh] relative bg-[#050505]">
            <div className="sticky top-0 h-screen overflow-hidden flex items-center justify-center">

                {/* Central Text */}
                <motion.div
                    style={{ scale: textScale, opacity: textOpacity }}
                    className="absolute z-50 text-center pointer-events-none"
                >
                    <h2
                        className="text-[8vw] md:text-[5vw] font-black text-transparent tracking-tighter"
                        style={{ WebkitTextStroke: '1px rgba(255, 255, 255, 0.3)' }}
                    >
                        MY WORK
                    </h2>
                </motion.div>

                {pictures.map(({ src, scale, className, rotation }, index) => (
                    <motion.div
                        key={index}
                        style={{ scale, opacity: opacityFade }}
                        className="absolute top-0 w-full h-full flex items-center justify-center pointer-events-none"
                    >
                        <div className={`relative ${className}`}>
                            <div
                                className="w-full h-full rounded-xl overflow-hidden border border-white/10 bg-neutral-900"
                                style={{ transform: `rotate(${rotation}deg)` }}
                            >
                                <img
                                    src={src}
                                    loading="lazy"
                                    className="object-cover w-full h-full opacity-85"
                                    alt="Work"
                                />
                            </div>
                        </div>
                    </motion.div>
                ))}

                <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_0%,#050505_90%)] pointer-events-none z-10" />
            </div>
        </div>
    );
}